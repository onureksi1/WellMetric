...
import { Sparkles, Clock, AlertTriangle, CheckCircle2, Wand2 } from 'lucide-react';
...
export default function NewSurveyPage() {
  ...
  React.useEffect(() => {
    const saveDraft = async () => {
      try {
        await client.put('/admin/surveys/draft', debouncedValues);
  ...
  React.useEffect(() => {
    const checkDraft = async () => {
      try {
        const { data: draft } = await client.get('/admin/surveys/draft');
        if (draft) {
          setHasDraft(draft);
        }
      } catch (e) {
  ...
  const handleAiGenerate = async (aiDto: any) => {
    try {
      setAiLoading(true);
      const { data: aiQuestions } = await client.post('/admin/surveys/ai-generate', aiDto);
          ...
          <div className="bg-navy text-white p-4 rounded-2xl shadow-2xl flex items-center justify-between border border-white/10 backdrop-blur-md bg-navy/90">
            ...
            <div className="flex gap-2">
              <button 
                type="button" 
                onClick={async () => {
                  await client.delete('/admin/surveys/draft');
                  setHasDraft(null);
                }}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 max-w-5xl mx-auto pb-32">
        ...
        <div className="sticky top-0 z-40 bg-gray-50/80 backdrop-blur-md pb-4 pt-4 -mt-4 border-b border-gray-100 flex flex-col sm:flex-row items-start sm:items-center gap-4 justify-between">
          <div className="flex items-center gap-4">
            <Link href="/admin/surveys" className="p-2 bg-white rounded-full border border-gray-200 hover:border-primary transition-colors text-gray-500 hover:text-primary">
              <ArrowLeft size={20} />
            </Link>
            <div>
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 sticky top-24 space-y-5">
              ...
              <label className="flex items-center gap-3 cursor-pointer bg-gray-50 p-3 rounded-xl border border-gray-200 hover:border-primary transition-colors mt-4">
                  ...
                  <input 
                    ...
                    onChange={(e) => {
                      if (!e.target.checked) {
                        ...
                        methods.setValue('is_anonymous', true);
                      }
                    }}
                    className="hidden" 
                  />
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] text-gray-500">Çalışan kimlikleri gizlenir</span>
                </div>
              </label>
                  ...
                  <div className="bg-white rounded-[32px] w-full max-w-sm p-8 shadow-2xl space-y-6">
                    ...
                    <div className="text-center space-y-2">
                      ...
                      <p className="text-sm text-gray-500 leading-relaxed font-medium">
                        Anonim olmayan anket çalışan hesap modunu zorunlu kılar. Bu anketi alan firmalar için <span className="font-bold text-orange-600">employee_accounts</span> otomatik açılır.
                      </p>
                    </div>
                    <div className="flex gap-3">
                      ...
                      <button 
                        type="button" 
                        onClick={() => {
                          methods.setValue('is_anonymous', false);
                          setShowAnonWarning(false);
                        }}
                        className="flex-1 py-3 bg-navy text-white rounded-xl font-bold shadow-lg shadow-navy/20"
                      >
                        Anladım, Devam Et
                      </button>
                    </div>
