          ...
          {data?.dimension_averages?.length > 0 && (
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h3 className="font-bold text-slate-900">{t('trainer_dashboard.dimension_avgs')}</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                {data.dimension_averages.map((dim: any) => (
                  <div key={dim.dimension} className="p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-blue-200 transition-colors">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-xl">{getDimensionIcon(dim.dimension)}</span>
                      <span className="text-sm font-bold text-slate-900">{dim.avg_score.toFixed(1)}</span>
                    </div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                      {t(`common.dimensions.${dim.dimension}`, { defaultValue: dim.dimension, ns: 'common' })}
                    </p>
                    <p className="text-[10px] text-slate-500 mt-1">{dim.company_count} {tc('company')}</p>
                  </div>
                ))}
              </div>
            </div>
