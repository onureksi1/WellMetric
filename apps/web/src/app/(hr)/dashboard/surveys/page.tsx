  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-navy">{t('dashboard.surveys.title')}</h1>
          <p className="text-sm text-gray-500">{t('dashboard.surveys.subtitle')}</p>
        </div>
        <div />
      </div>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {surveys?.filter((s: any) => s.status === 'active').map((survey: any) => (
                ...
                <div>
                  ...
                  {survey.campaign_count === 0 && (
                    <div className="p-3 bg-warning/5 rounded-lg border border-warning/10 mb-4 flex gap-2 items-center text-warning text-[10px] sm:text-xs font-bold uppercase tracking-widest">
                      <AlertCircle size={16} />
                      {t('dashboard.surveys.no_distribution')}
                    </div>
                  )}

                  <div className="space-y-4">
                    <div className="flex justify-between items-end">
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t('dashboard.surveys.participation_status')}</p>
                      <p className="text-xs sm:text-sm font-black text-navy uppercase tracking-widest">{survey.campaign_count > 0 ? t('dashboard.surveys.distributed') : t('common.waiting')}</p>
                    </div>
                ...
                <div className="mt-8 flex flex-col sm:flex-row gap-2">
                  <Link href={`/dashboard/surveys/${survey.survey_id}/results`} className="flex-1">
                    <Button variant="secondary" size="sm" className="w-full gap-2 font-bold py-2.5">
                      <BarChart2 size={16} />
                      {t('dashboard.surveys.view_results')}
                    </Button>
                  </Link>
                  {survey.status === 'active' && survey.campaign_count === 0 && (
                    ...
                    >
                      <Send size={16} />
                      {t('dashboard.surveys.start_distribution')}
                    </Button>
                  )}
                  {survey.campaign_count > 0 && (
            ...
            {surveys?.filter((s: any) => s.status === 'active').length === 0 && (
              <Card className="border-l-4 border-l-warning flex items-center justify-center bg-gray-50/50 border-dashed lg:col-span-2 py-12">
                <div className="text-center">
                  ...
                  <p className="text-sm font-black text-gray-400 uppercase tracking-widest">{t('dashboard.surveys.no_active_survey')}</p>
                </div>
              </Card>
            )}
          </div>
          ...
          {/* Completed Surveys */}
          <Card title={t('dashboard.surveys.past_surveys')}>
            ...
            <div className="p-0 overflow-x-auto">
              ...
              <table className="hidden md:table w-full text-left text-sm">
                <thead className="bg-gray-50/50 text-gray-400 font-bold uppercase tracking-widest text-[10px]">
                  <tr>
                    <th className="py-4 px-4">{t('dashboard.surveys.survey_title')}</th>
                  ...
                  {surveys?.filter((s: any) => s.status !== 'active').map((survey: any) => (
                    <tr key={survey.id} className="hover:bg-gray-50/30 transition-colors group">
                      ...
                      <td className="py-4 px-4 text-right">
                        <Button variant="ghost" size="sm" className="text-primary font-black hover:bg-primary/5 uppercase tracking-widest text-[10px]">{t('dashboard.surveys.view_results')}</Button>
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              {surveys?.filter((s: any) => s.status !== 'active').length === 0 && (
                <div className="py-12 text-center text-gray-400 italic text-sm">{t('dashboard.surveys.no_past_surveys')}</div>
              )}
            </div>
          </Card>
        </>
