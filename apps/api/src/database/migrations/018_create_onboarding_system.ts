...
export class CreateOnboardingSystem1746600000000 implements MigrationInterface {
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      ...
      VALUES 
        ('${wave1Id}', 'İlk gününüzde ihtiyacınız olan tüm ekipmanlar hazır mıydı?', 'Was all the equipment you needed ready on your first day?', 'yes_no', 1, 'physical'),
        ('${wave1Id}', 'Ekibinizle tanışma süreci nasıl geçti?', 'How was the process of meeting your team?', 'likert5', 2, 'social'),
        ('${wave1Id}', 'İlk gün deneyiminizi iyileştirmek için ne yapabilirdik?', 'What could we have done to improve your first day experience?', 'open_text', 3, 'mental');
    `);
    ...
    await queryRunner.query(`
      INSERT INTO survey_questions (survey_id, question_text_tr, question_text_en, question_type, order_index, dimension)
      VALUES 
        ('${wave2Id}', 'İş sorumluluklarınız size net bir şekilde aktarıldı mı?', 'Were your job responsibilities clearly communicated to you?', 'likert5', 1, 'mental'),
        ('${wave2Id}', 'Şirket kültürüne uyum sağladığınızı hissediyor musunuz?', 'Do you feel that you have adapted to the company culture?', 'likert5', 2, 'social'),
        ('${wave2Id}', 'Şu ana kadar yaşadığınız en büyük zorluk nedir?', 'What is the biggest challenge you have experienced so far?', 'open_text', 3, 'mental');
    `);
    ...
    await queryRunner.query(`
      INSERT INTO survey_questions (survey_id, question_text_tr, question_text_en, question_type, order_index, dimension)
      VALUES 
        ('${wave3Id}', 'Rolünüzde başarılı olmak için yeterli destek alıyor musunuz?', 'Are you receiving enough support to succeed in your role?', 'likert5', 1, 'mental'),
        ('${wave3Id}', 'Gelecekte bu şirkette kendinizi nerede görüyorsunuz?', 'Where do you see yourself in this company in the future?', 'open_text', 2, 'mental');
    `);
  }
